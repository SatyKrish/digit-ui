...
export class MessageRepository {
  ...
  getMessageById(id: string): ChatMessage | null {
    const stmt = this.getDb().prepare('SELECT * FROM chat_messages WHERE id = ?');
  ...
  deleteMessage(id: string): boolean {
    const stmt = this.getDb().prepare('DELETE FROM chat_messages WHERE id = ?');
  ...
  deleteAllMessagesForSession(sessionId: string): number {
    const stmt = this.getDb().prepare('DELETE FROM chat_messages WHERE session_id = ?');
  ...
  getMessageCountForSession(sessionId: string): number {
    const stmt = this.getDb().prepare('SELECT COUNT(*) as count FROM chat_messages WHERE session_id = ?');
